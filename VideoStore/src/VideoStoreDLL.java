...
public class VideoStoreDLL implements VideoStoreInterface{
    private final DList videosInStore = new DList();
    private final DList customers = new DList();
    ...
    public Video getVideoById(int videoId) {
        ...
        for (int i = 0; i < customers.size(); i++) {  // Runs through each customer
            ...
            DList rentedVideos = (DList) currentCustomer.getRentedVideos();
    ...
    public void addCustomer(String customer) {
        customers.add(new Customer(customer, nextId++, Customer.ListType.DLIST));
    }
    ...
    public boolean checkIn(int videoId) {
        for (int i = 0; i < customers.size(); i++) {
            ...
            DList rentedVideos = (DList)currCustomer.getRentedVideos();
    ...
    public void printAllRentedVideos() {
        for (int i = 0; i < customers.size(); i++) {
            Customer currentCustomer = (Customer) customers.get(i);
            DList videos = (DList) currentCustomer.getRentedVideos();
            if (videos.size() > 0) {
                videos.print();
            }
        }
